...
class WebonyxSchemaBuilder extends SchemaBuilder
{
    public function buildFromTypeLanguage(string $typeLanguage): Schema
    {
        $typeLanguage = BuildSchema::build($typeLanguage);
        
        return new Schema(
            collect($typeLanguage->getTypeMap())->filter(function (WebonyxType $type) {
    ...
    public function toType(WebonyxType $type): Type
    {
        ...
        switch (true) {
            case $type instanceof WebonyxObjectType:
                return new ObjectType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxIDType:
                return new IDType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxStringType:
                return new StringType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxFloatType:
                return new FloatType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxIntType:
                return new IntType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxBooleanType:
                return new BooleanType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxScalarType:
                return new ScalarType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxEnumType:
                return new EnumType(
                    $type->name,
                    $type->description,
            ...
            case $type instanceof WebonyxNonNullType:
                return new NonNullType(
                    $this->toType($type->getWrappedType(false))
                );
            case $type instanceof WebonyxListOfType:
                return new ListType(
                    $this->toType($type->getWrappedType(false))
                );
            case $type instanceof WebonyxInterfaceType:
                return new InterfaceType(
                    $type->name,
                    $type->description,
                    $fields
                );
            case $type instanceof WebonyxInputObjectType:
