...
class ForceDeleteDirective extends BaseDirective implements FieldResolver
{
    /**
            ...
            function ($root, array $args) {
                ...
                if ($argumentDefinition->type->kind !== NodeKind::NON_NULL_TYPE) {
                    throw new DirectiveException(
                        "The @forceDelete directive requires the field {$this->definitionNode->name->value} to have a NonNull argument. Mark it with !"
                    );
                }
    ...
    protected function getSingleArgumentDefinition(): InputValueDefinitionNode
    {
        if (count($this->definitionNode->arguments) !== 1) {
            throw new DirectiveException(
                "The @forceDelete directive requires the field {$this->definitionNode->name->value} to only contain a single argument."
            );
        }
