...
## Resolving root Query type

It's advisable to create a result type for each mutation. It should contain the mutation result and the `Query` object.
This will allow the client in one round-trip not only to call mutation and evaluate result but also to get a wagon of data
to update their application.

For example, we have a mutation:
...
Lighthouse automatically resolves the `Query` type. So the mutation resolver will look like this:
