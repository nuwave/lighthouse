...
in the default model namespace `App\Models`. [You can change this configuration](configuration.md). 
...
the built in directives for [querying data](fields.md#query-data) and [mutating data](fields.md#mutate-data),
as well as the convention based approach to [implementing custom resolvers](fields.md#custom-resolver).
...
of the default namespaces that are defined in the [configuration](configuration.md). The following
...
Make sure you read the [basics about Interfaces](types.md#interface) before deciding
...
If you need to apply middleware to a group of fields, you can put [@middleware](directives.md#middleware) on an Object type.
...
Other then global middleware defined in the [configuration](configuration.md), field middleware
...
Make sure you read the [basics about Unions](types.md#union) before deciding
to use this directive, you probably don't need it.
